                ...
                <div class="card-body">
                    <form action="{{ route('response.orders.confirm', $order) }}" method="POST">
                        ...
                        @if ($order->status->name === 'pending')
                            <button type="submit" class="btn btn-success w-100 mb-3">تأكيد الطلب</button>
                        @else
                            <button type="submit" class="btn btn-success w-100 mb-3" disabled>تم التأكيد</button>
                        @endif>
                    </form>
                    ...
                    @if ($order->status->name !== 'delivered')
                        <form action="{{ route('response.orders.assignPreparation', $order) }}" method="POST">
                            ...
                            @method('PUT')
                            <div class="mb-3">
                                <label for="preparation_employee_id" class="form-label">تعيين موظف التجهيز</label>
                                <select name="preparation_employee_id" id="preparation_employee_id" class="form-select"
                                    required>
