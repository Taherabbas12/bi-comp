            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h4>تعديل الحالة: {{ $taskStatus->display_name }}</h4>
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.task-statuses.update', $taskStatus) }}">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label for="name" class="form-label">الاسم (مفتاح فريد)</label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    id="name" name="name" value="{{ old('name', $taskStatus->name) }}" required>
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('display_name') is-invalid @enderror"
                                    ...
                                    value="{{ old('display_name', $taskStatus->display_name) }}" required>
                                @error('display_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="color"
                                    ...
                                    value="{{ old('color_code', $taskStatus->color_code) }}">
                                @error('color_code')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
